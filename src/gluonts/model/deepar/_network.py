...
class DeepARTrainingNetwork(DeepARNetwork):
    ...
    def hybrid_forward(
        ...
        if future_target is not None:
            ...
            repeated_time_feat = future_time_feat.repeat(
                repeats=num_parallel_samples, axis=0
            )
            ...
            repeated_states = [
                s.repeat(repeats=num_parallel_samples, axis=0)
                for s in begin_states
            ]

            future_samples = []
            shifted_lags = [l - 1 for l in self.lags_seq]
            ...
            for k in range(self.prediction_length):
                ...
                # prod(target_shape) * num_lags + num_time_features + num_static_features)
